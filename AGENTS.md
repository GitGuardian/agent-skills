...
| [`scan-machine`](skills/scan-machine/SKILL.md) | Scan the entire developer machine for credentials across local git repositories, dotfiles, cloud CLI configs, shell history, AI agent caches, and abandoned project trees. **Requires GitGuardian Growth tier or higher** (endpoint scanning is gated server-side; not available on Free). |
