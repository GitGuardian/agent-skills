...
- `scan-secrets/references/remediation-doctrine.md` is the **slim core** of the doctrine and acts as the router: the four triage axes, the four deliverable modes, implementation profiles, the §9.0 long-tail schema, the generic coordination framework, public-leak takedown, per-mode validation, and the §13 custom-remediation-workflow overlay. The bulk is split into progressive-disclosure siblings, each one hop from `SKILL.md`: `remediation-lifecycle-tracks.md` (§5–8 lifecycle tracks: pre-leak, post-leak public, post-leak internal-private, off-repo), `remediation-cloud-keys.md` (§9.1/9.8/9.9), `remediation-saas-tokens.md` (§9.2/9.3/9.6/9.7/9.10), and `remediation-keys-and-dbs.md` (§9.4/9.5/9.11/9.12). §-numbering is preserved across the split, so a `§ N` cross-reference resolves to the same place regardless of which file it lives in. Two sections stay core-local on purpose: §13, because a configured customer workflow overlays *every* track and mode rather than belonging to one, and the §1 principle 7 / §3 "validity does not select a mode" pair, because validity gates triage itself. The twelve §9 worked examples cover credential *archetypes* (overlap support, revoke-vs-regenerate asymmetry, signing-secret invalidation, self-expiry, absent control plane), not every detector; a new archetype is a focused diff in one family file. `SKILL.md` routes into the core first, then the relevant track + credential-family file when findings are present; the sibling `interpreting-results.md` is a *separate* reference for reading ggshield output (JSON shape, validity, severity), the HMSL handoff, and `ggignore` false positives — it is not a remediation file.
- `triage-incidents/references/remediation-doctrine.md` and its four siblings — the same five-file structure, tailored to a post-leak-only detection context and kept **structurally diffable** against the `scan-secrets` set. The intended divergences are exactly three: §5 (pre-leak) is a not-applicable stub, §13 covers the Incident-page touchpoint (the `get_remediation_workflow` `workflow.id` gate) rather than the pre-leak ggshield channel, and principle 7 / "validity does not select a mode" are worded for incidents rather than scan findings. Anything else that drifts between the two sets is a bug.
