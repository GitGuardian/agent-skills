...
`kiro/skills/` mirrors the published `SKILL.md` and `references/` files in `skills/` byte-for-byte, including release annotations. Copy changed files and remove retired references in both trees. `npm run check:references` checks local links, anchors, skill isolation and Kiro parity, including missing or stale copies; CI runs the same check.
