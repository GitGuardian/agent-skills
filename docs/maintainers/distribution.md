...
- **Filename and schema matter for MCP config**: Cursor specifically looks for `mcp.json` (not `.mcp.json`), Claude Code reads `.mcp.json` with `mcpServers`, and Codex uses `.codex-mcp.json` pointed to by `.codex-plugin/plugin.json` with the `mcp_servers` wrapper accepted by Codex. Claude Code and Cursor configs use `"type": "http"` pointing at the hosted server; Codex config uses local stdio via `uvx` with the unified `gg-mcp-server` entrypoint (the deprecated `developer-mcp-server` and `secops-mcp-server` scripts will be removed in a future ggmcp release).
