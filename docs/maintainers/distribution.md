...
| `.codex-mcp.json` (Codex) + `.mcp.json` (Claude) + `mcp.json` (Cursor) | plugin root | ✅ | Auto-configure the GitGuardian MCP server on install |
...
- **Filename and schema matter for MCP config**: Cursor specifically looks for `mcp.json` (not `.mcp.json`), Claude Code reads `.mcp.json` with `mcpServers`, and Codex uses `.codex-mcp.json` pointed to by `.codex-plugin/plugin.json` with the `mcp_servers` wrapper accepted by Codex. All three point at the hosted server, but the HTTP schema differs by client: Claude Code and Cursor use `"type": "http"` with `"url"`, while Codex uses a bare `"url"` field (no `type` key) and triggers OAuth via a one-time `codex mcp login GitGuardian` rather than the automatic browser flow the other two use. Codex's deprecated stdio entrypoints (`developer-mcp-server` and `secops-mcp-server`, replaced by the unified `gg-mcp-server`) will be removed in a future ggmcp release; the stdio mode itself remains available for self-hosted or PAT/CI use.
