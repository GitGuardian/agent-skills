...
Apply this framework to already-detected incidents. Every incident is an exposure. Read it before remediation advice; retry incomplete reads in smaller ranges and stop the plan if it remains unavailable.
...
This skill uses the incident-connected profile. Read the available incident evidence and company workflow; ask for missing authority or runtime facts.

## 5. Pre-leak track

Not applicable to this skill: every incident is already an exposure. A current clean checkout, a workstation location or an unpushed commit does not make an incident pre-leak.
...
An already-detected incident remains exposed on a workstation or other off-repo surface; location changes the cleanup owner and method, not the need to assess invalidation.
...
Resolve only after verified remediation and user confirmation. Intent, TTL alone or an unchecked validity label is insufficient. For unverifiable validity, offer the user-run HMSL handoff in [SKILL.md](../SKILL.md).

## 13. Custom remediation workflows (the organizational overlay)

Fetch `get_remediation_workflow` and announce custom/default. Only `workflow.id` identifies a custom workflow. With an ID, preserve every returned title, description, link label and URL. Without an ID, set returned default steps aside and apply this framework. If unavailable, state that limitation and use the framework.
