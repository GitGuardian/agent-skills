...
- **Treat a match as confirmation, not coincidence.** HMSL's corpus is built from indexed public sources — a match means GitGuardian saw your exact secret in a public artifact. Every match is a finding for the post-leak / public-facing track of the doctrine ([`references/remediation-doctrine.md`](references/remediation-doctrine.md) § 6) — walk the user through that triage flow.
