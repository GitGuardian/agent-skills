...
- **Execution model: user-run only.** `ggshield machine scan` walks every file in `$HOME` and surfaces live credentials across cloud CLIs, browser profiles, shell history, and AI agent caches. Even with `--show-findings`, the JSON output contains plaintext or near-plaintext credential values, which would land in the agent's transcript. The agent prepares the exact commands and explains the trade-offs; the **user runs them in their own terminal** and either pastes back a sanitized summary (counts by detector + validity), browses results in `ggshield machine dashboard`, or shares specific findings the user has chosen to surface. The agent does **not** invoke `ggshield machine scan`, `ggshield machine inventory`, or read the local findings database (`~/.ggshield/ggshield_machine_scan.db`). Agent-executable commands in this skill are limited to non-credential-bearing operations: `ggshield --version`, `ggshield api-status`, `ggshield plugin status`, `ggshield plugin list`, `ggshield machine --help`.
