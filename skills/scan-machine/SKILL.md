...
For remediation once findings are surfaced: a live credential is rotated first, then the on-disk copy is removed; a dead one is deleted in place. A found credential is a found credential regardless of which scanner found it. If the `scan-secrets` skill is installed, its remediation doctrine carries the off-repo exposure track and the per-secret-type runbooks; load it for those.
...
- **Treat findings by validity:** live credentials need rotation now; dead credentials can be deleted in place. Walk the user through both per finding; if the `scan-secrets` skill is installed, its remediation doctrine has the off-repo track and per-type runbooks.
