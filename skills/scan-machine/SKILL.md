...
For remediation guidance once findings are surfaced, see [`references/remediation-doctrine.md`](references/remediation-doctrine.md) — the cross-skill GitGuardian Remediation Doctrine. Machine-scan findings dispatch to the off-repo track (§ 8); the doctrine covers triage axes, deliverable modes, per-secret-type rotation runbooks, and validation. A found credential is a found credential regardless of which scanner found it.
...
- **Treat findings by validity:** live credentials need rotation now; dead credentials can be deleted in place. Walk the user through both per finding — see [`references/remediation-doctrine.md`](references/remediation-doctrine.md) § 8 (off-repo exposure track) for the triage flow.
