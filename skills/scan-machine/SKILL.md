...
For remediation once findings are surfaced: a found credential is a found credential. Treat live credentials as compromised — rotate them at the issuing service and update every system that consumes them; remove dead credentials in place. Machine findings sit off-repo (dotfiles, shell history, CLI configs), so also scrub the credential from the location it was found. How urgent the rotation is depends on the machine (a shared / corp-issued machine or a backed-up home directory widens the blast radius past you) and on what the credential unlocks.
...
- **Treat findings by validity:** live credentials need rotation now; dead credentials can be deleted in place. Walk the user through both per finding — rotate the live ones at the issuing service and scrub every credential from the off-repo location it was found in.
