...
- **`workflow.id` absent → GitGuardian default workflow → no overlay, and the default steps are discarded.** Do **not** render, quote, or use them as scaffolding. Drive end-to-end from the doctrine as in [§§ 6–12](remediation-lifecycle-tracks.md#6-post-leak--public-facing-track), exactly as if the tool had returned nothing. The default `steps[]` are GitGuardian's generic placeholder, not workspace-authored guidance — they add nothing the doctrine does not already cover more completely, so the doctrine is the deliverable in full.
